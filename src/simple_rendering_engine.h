// src/main.cpp - Fixed Clickable UI Controls
...
void showLeftPanel() {
    ...
    if (showTransformWindow && selectedObjectIndex >= 0 && selectedObjectIndex < objects.size()) {
        if (ImGui::CollapsingHeader("Transform", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            // Transform mode buttons with improved styling
            ImGui::Text("Mode:");
            ImGui::SameLine();
            
            // Use colored buttons for transform modes
            ImGui::PushStyleColor(ImGuiCol_Button, transformMode == TRANSFORM_TRANSLATE ? COLOR_BUTTON_ACTIVE : COLOR_BUTTON);
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, transformMode == TRANSFORM_TRANSLATE ? COLOR_BUTTON_ACTIVE : COLOR_BUTTON_HOVER);
            ...
            ImGui::PushStyleColor(ImGuiCol_Button, transformMode == TRANSFORM_ROTATE ? COLOR_BUTTON_ACTIVE : COLOR_BUTTON);
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, transformMode == TRANSFORM_ROTATE ? COLOR_BUTTON_ACTIVE : COLOR_BUTTON_HOVER);
            ...
            ImGui::PushStyleColor(ImGuiCol_Button, transformMode == TRANSFORM_SCALE ? COLOR_BUTTON_ACTIVE : COLOR_BUTTON);
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, transformMode == TRANSFORM_SCALE ? COLOR_BUTTON_ACTIVE : COLOR_BUTTON_HOVER);
            ...
            ImGui::PushStyleColor(ImGuiCol_ButtonHovered, ImVec4(1.0f, 0.5f, 0.4f, 1.0f));
            ImGui::PushStyleColor(ImGuiCol_ButtonActive, ImVec4(1.0f, 0.3f, 0.2f, 1.0f));
...
void showRightPanel() {
    ...
    if (showLightWindow) {
        if (ImGui::CollapsingHeader("Lighting", ImGuiTreeNodeFlags_DefaultOpen)) {
            ...
            bool wireframeChanged = ImGui::Checkbox("Wireframe", &wireframeMode);
            bool gridChanged = ImGui::Checkbox("Show Grid", &showGrid);
            bool axesChanged = ImGui::Checkbox("Show Axes", &showAxes);
            bool bboxChanged = ImGui::Checkbox("Show Bounding Boxes", &showBoundingBoxes);
            
            if (wireframeChanged) {
                snprintf(statusMessage, sizeof(statusMessage), 
                         wireframeMode ? "Wireframe mode enabled" : "Wireframe mode disabled");
            }
            if (gridChanged) {
                snprintf(statusMessage, sizeof(statusMessage), 
                         showGrid ? "Grid shown" : "Grid hidden");
            }
            if (axesChanged) {
                snprintf(statusMessage, sizeof(statusMessage), 
                         showAxes ? "Axes shown" : "Axes hidden");
            }
            if (bboxChanged) {
                snprintf(statusMessage, sizeof(statusMessage), 
                         showBoundingBoxes ? "Bounding boxes shown" : "Bounding boxes hidden");
            }
        }
    }
...
void showViewport() {
    ...
    if (ImGui::Begin("Viewport", NULL, 
        ...
        if (viewportTexture != 0) {
            // Use InvisibleButton to capture mouse input without blocking ImGui
            ImGui::InvisibleButton("viewport_canvas", contentSize);
            
            // Display texture on top of the invisible button
...
void mouse_button_callback(GLFWwindow* window, int button, int action, int mods) {
    // Let ImGui handle mouse events first
...
void key_callback(GLFWwindow* window, int key, int scancode, int action, int mods) {
    ...
    // Let ImGui handle keyboard events first
    ...
    switch (key) {
        ...
        // Toggle view options with feedback
        case GLFW_KEY_Z:
...
void drop_callback(GLFWwindow* window, int count, const char** paths) {
    // Let ImGui handle drag and drop events first
    if (ImGui::GetIO().WantCaptureMouse) return;
